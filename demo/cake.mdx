...
```srt
we get an authentication error. So this super lightweight method works alright if we're just surfacing our embeds to people that know what Looker is, but it doesn't let us actually build an app or experience around Looker.
...
<Browser url="/embedsdk" loadUrl="/04" zoom={0.8} />
<Editor
  code="04.js"
  focus="14:20"
  lang="jsx"
...
```srt
so lets shake things up a bit and bring in the Looker Embed SDK to build some more robust embeds. We'll start this up inside a small React App.
95.5 -> 100.5
and React elements
